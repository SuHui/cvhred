...
class base_enc_dec:
    ...
    def __init__(self, labels, alabel, elabel, length, h_size, vocab_size, embedding, batch_size, learning_rate, mode,beam_size=5):
        ...
        self.alabel = tf.concat(0,[alabel,tf.zeros([1, batch_size], dtype=tf.int64)])
        self.elabel = tf.concat(0,[elabel,tf.zeros([1, batch_size], dtype=tf.int64)])
        if mode == 2:
            self.alabel = alabel
            self.elabel = elabel
    ...
    def scan_step(self):
        ...
        _, h_d,_,_ = tf.scan(self.run, [self.labels,self.alabel,self.elabel], initializer=[init_encode, init_decoder,tf.zeros([self.batch_size,1]),tf.zeros([self.batch_size, 1])])#tf.cast(0,tf.int64),tf.cast(0,tf.int64)])
        return [_, h_d]
